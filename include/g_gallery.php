    ...
    case "add_images":{
    	if ( !empty($_FILES) ){
			for ($i=0;$i<5;$i++){
				...
				db_query("INSERT INTO $sql_tbl[gallery] (md5,date,category,alt,avail) VALUES('catalogue','".time()."','$image_cat','$alt','N')");
				$id=db_insert_id();
...
if (is_array($images))
	foreach ($images as $a=>$v){
		...
		if ($v["icon"]!='')	$images[$a]["icon"]="img/gallery/".$v['id']."_icon.".$v["icon"];
		else $images[$a]["icon"]="img/gallery/icons/".$v['id']."_image.".$v["image"];
	}
?>