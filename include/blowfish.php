...
class ctBlowfish {
	...
	function bf_enc($ll, $r, $n) {
		...
		# XXX & 0xffffffff ueberall? Wenn int zu gross wird, wird es in
		# float konvertiert, und das koennte schlecht sein, wegen
		# genauigkeits- und geschwindigkeitsverlust
		return $ll ^ ((($b1 + $b2) ^ $b3) + $b4);
	}
	...
	function ctEncrypt($plaintext, $len, $password) {
		...
		if (strlen($plaintext) != $len) {
			$this->eraseVars();
			echo '$plaintext length don\'t match with $len';
			return false;
		}
	...
	function ctDecrypt($cipher, $len, $password) {
		$this->createVars();
		if (strlen($password) <= 0) $password = ' ';
		# hex nach bin konvertieren
		$cipher = pack('H' . strlen($cipher), $cipher);
# by max - if we don't know length...?
		$len = strlen($cipher);
		if ($len > strlen($cipher)) {
			$this->eraseVars();
			echo 'Restored text length don\'t match.\n';
			return false;
		}
		$this->bf_set_key($password);
		$return = substr($this->decrypt($cipher), 0, $len);
		$this->eraseVars();
		return $return;
	}
}
...
function func_bf_crypt($s, $key) {
	...
	$s = $blowfish->expand($s);
	return $blowfish->ctEncrypt($s, strlen($s), $key);
}
...
function func_bf_decrypt($s, $key) {
    ...
    if(!$blowfish)
        return false; 
	return trim($blowfish->ctDecrypt($s, strlen($s), $key));
}

function func_bf_asc($array, $key) {
	global $blowfish;
	$len = count($array); $return = "";
	for ($i=1; $i < $len; $i++) {
		$return .= $blowfish->int2asc($array[$i]);
	}
	$return = func_bf_decrypt($return, $key);
	if (crc32($return) != (int)$array[0]) return false;
